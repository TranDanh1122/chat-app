    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3.5 w-4/5 md:w-2/3 lg:w-1/3 bg-[#FFFFFF]/5 px-10 py-14 rounded-2xl max-w-[400px]">
                ...
                <Button className="text-sm size-auto w-full py-4 flex hover:opacity-50 cursor-pointer rounded-4xl mt-6" type="submit">Reset password</Button>
                <p className="text-[0.75rem] text-white leading-4 opacity-80 text-center">
                    Got your password?  <Link className="font-semibold" to="/auth/register">Sign in</Link>
                </p>
            </form>
        </Form>
    )
